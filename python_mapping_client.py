...
class RobotMapper:
    ...
    def update_log_odds(self, cell_num, probability):
        ...
        # Update log odds
        self.grid[cell_y, cell_x] += log_odds + self.initial_map_log_odds
    ...
    def get_distance_measurement(self):
        ...
        if data:
            print(f"[SENSOR DATA] Distance: {data} mm")
            distance_cm = float(data) / 10  # Convert mm to cm
            self.process_distance(distance_cm)

    def get_dummy_distance_measurement(self):
        # Dummy distance function for testing
        distance_cm = float(input("Enter dummy distance measurement (cm): "))
        self.process_distance(distance_cm)

    def print_grid_states(self):
        print("\nGrid States:")
        print("Time\tState\tOrientation")
        for t, grid_state in enumerate(self.grid_states):
            print(f"{t}\t{grid_state}\t{self.orientation}")
    ...
    def run(self):
        while True:
            ...
            print("4. Get dummy distance measurement")
            print("5. Print grid states")
            print("6. Exit")
            ...
            if choice == '1':
                ...
                self.get_dummy_distance_measurement()
            elif choice == '5':
